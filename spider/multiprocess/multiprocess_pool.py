...
"""
multiprocess_pool.py by YunofHD
"""
...
class MultiprocessPool(object):
    ...
    def __init__(self, fetcher, parser=None, saver=None, proxieser=None, url_filter=None, max_count=500, max_count_in_proxies=100):
        ...
        self._queue_fetch = queue.PriorityQueue()           # (priority, counter, url, keys, deep, repeat) 优先度（指整个爬虫的深度），计数器，网址，键，深度，重复次数
        ...
        self._multiprocess_fetcher_list = []                      # fetcher thread list
        self._multiprocess_parser = None                          # parser thread
        self._multiprocess_saver = None                           # saver thread
        self._multiprocess_proxieser = None                       # proxieser thread

        self._multiprocess_stop_flag = False                      # default: False, stop flag of threads
        ...
        self._lock = multiprocessing.Lock()                       # the lock which self._number_dict needs
    ...
    def start_working(self, fetcher_num=10):
        ...
        self._multiprocess_stop_flag = False

        self._multiprocess_fetcher_list = [FetchMultiprocess("fetcher-%d" % (i+1), copy.deepcopy(self._inst_fetcher), self, max_count=self._max_count, daemon=True) for i in range(fetcher_num)]
        self._multiprocess_parser = ParseMultiprocess("parser", self._inst_parser, self, daemon=True) if self._inst_parser else None
        self._multiprocess_saver = SaveMultiprocess("saver", self._inst_saver, self, daemon=True) if self._inst_saver else None
        self._multiprocess_proxieser = ProxiesMultiprocess("proxieser", self._inst_proxieser, self, max_count=self._max_count_in_proxies, daemon=True) if self._inst_proxieser else None

        for process in self._multiprocess_fetcher_list:
            # thread.setDaemon(True)
            process.start()

        if self._multiprocess_parser:
            # self._thread_parser.setDaemon(True)
            self._multiprocess_parser.start()

        if self._multiprocess_saver:
            # self._thread_saver.setDaemon(True)
            self._multiprocess_saver.start()

        if self._multiprocess_proxieser:
            # self._thread_proxieser.setDaemon(True)
            self._multiprocess_proxieser.start()
